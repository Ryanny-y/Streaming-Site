  ...
  useEffect(() => {
    ...
    if(filmType && showId) {
      const getReviews = async () => {
        try {
          setIsLoading(true);
          const response = await fetch(`https://api.themoviedb.org/3/${filmType}/${showId}/reviews`, {
            method: 'GET',
            headers: {
          ...
          reviews.map(review => 
            <div key={review.id} className="flex gap-8">
              <div id="avatar_poster" className="h-40 w-40 shrink-0">
                <img src={`https://image.tmdb.org/t/p/w500${review.author_details.avatar_path}`} alt="Avatar Photo" className="h-full w-full rounded-full text-center"/>
              </div>

              <div id="review_details" className="flex flex-col gap-2">
                <h1 id="author_name">{review.author}</h1>
                <p id="last_updated">{dayjs(review.updated_at).format('MM/DD/YYYY')}</p>
                  ...
                  : 
                    <>
                      <span onClick={() => setFullReview(true)}>
                        {review.content.slice(0, 150).concat(' See More...')}
                      </span>
                    </>
                  }
                </p>
